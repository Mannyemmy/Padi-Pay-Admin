...
export default function BrmAgentsPage() {
  ...
  const [editIdPhotoFile, setEditIdPhotoFile] = useState<File | null>(null);
  const [editProfilePhotoPreview, setEditProfilePhotoPreview] = useState<string | null>(null);
  const [editIdPhotoPreview, setEditIdPhotoPreview] = useState<string | null>(null);

  function openEditModal(brm: Brm) {
    setEditTarget(brm);
    setEditForm({
    ...
    setEditIdPhotoFile(null);
    setEditProfilePhotoPreview(brm.profile_photo_url ?? null);
    setEditIdPhotoPreview(brm.id_photo_url ?? null);
  }
  ...
  async function handleEditBrm(e: React.FormEvent) {
    ...
    setEditLoading(true);
    try {
      let profilePhotoUrl = editTarget.profile_photo_url ?? undefined;
      let idPhotoUrl = editTarget.id_photo_url ?? undefined;

      if (editProfilePhotoFile) {
        try {
          profilePhotoUrl = await uploadBrmFile(editProfilePhotoFile, editTarget.id, 'profile_photo');
        } catch (uploadErr) {
          console.error('Failed to upload profile photo:', uploadErr);
          showToast('warning', 'Photo upload failed', 'Could not update profile photo');
        }
      }

      if (editIdPhotoFile) {
        try {
          idPhotoUrl = await uploadBrmFile(editIdPhotoFile, editTarget.id, 'id_photo');
        } catch (uploadErr) {
          console.error('Failed to upload ID photo:', uploadErr);
          showToast('warning', 'Photo upload failed', 'Could not update ID photo');
        }
      }
  ...
  async function handleOverrideActivation() {
    ...
    try {
      ...
      showToast('success', 'Activated', 'POS agent activated & ₦5,000 bonus credited');
    } catch {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="BRM Agents"
        description="Manage Business Relationship Managers — commissions, cashouts, and POS agent activations"
        action={{
        ...
        <nav className="flex gap-6">
          {(
            [
              ...
              { id: 'activations', label: 'POS Agent Activations', count: merchants.length },
          ...
          {tab === 'agents' && (
            <div className="card space-y-4">
              ...
              {filteredBrms.length === 0 ? (
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-200/80 dark:border-gray-700/80 bg-white dark:bg-gray-900">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[980px] text-sm">
                    <thead>
                      <tr className="text-left text-[11px] tracking-wide uppercase text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700 bg-gray-50/70 dark:bg-gray-800/60">
                        <th className="py-3 pl-4 pr-3 font-semibold">Name</th>
                        <th className="py-3 px-3 font-semibold">Phone</th>
                        <th className="py-3 px-3 font-semibold hidden md:table-cell">State</th>
                        <th className="py-3 px-3 font-semibold hidden lg:table-cell">Referral Code</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                      {filteredBrms.map((brm) => {
                        ...
                        const nameParts = brm.full_name?.trim().split(' ') ?? [];
                        const initials =
                          (nameParts[0]?.[0] ?? '') +
                          (nameParts.length > 1 ? nameParts[nameParts.length - 1]?.[0] ?? '' : '');
                        return (
                          <Fragment key={brm.id}>
                            <tr className="group hover:bg-blue-50/40 dark:hover:bg-blue-900/10 transition-colors">
                              <td className="py-3 pl-4 pr-3">
                                <div className="flex items-center gap-3">
                                  <button
                                    ...
                                    className="cursor-pointer text-gray-400 hover:text-blue-600 rounded-md border border-transparent hover:border-blue-200 dark:hover:border-blue-700 p-0.5 transition-colors"
                                  >
                                    {isExpanded ? (
                                  ...
                                  <div className="h-9 w-9 rounded-full bg-gradient-to-br from-blue-100 to-cyan-100 dark:from-blue-900/40 dark:to-cyan-900/40 text-blue-700 dark:text-blue-300 flex items-center justify-center text-xs font-semibold">
                                    {initials || 'BR'}
                                  </div>
                                  <div>
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{brm.email}</p>
                                  </div>
                                </div>
                              </td>
                              ...
                              <td className="py-3 px-3 text-gray-600 dark:text-gray-300 hidden md:table-cell">
                                {brm.state || '—'}
                              </td>
                              <td className="py-3 px-3 font-mono text-xs text-gray-500 hidden lg:table-cell">
                              ...
                              <td className="py-3 px-3">
                                ...
                                >
                                  {brm.status === 'active' ? 'Active' : 'Suspended'}
                                </span>
                              </td>
                              <td className="py-3 pl-3 pr-4 text-right">
                                <div className="flex items-center justify-end gap-2">
                                  ...
                                  <button
                                    ...
                                    className="text-xs px-3 py-1.5 rounded-lg border border-blue-200 text-blue-700 bg-blue-50/70 hover:bg-blue-100 dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-300 dark:hover:bg-blue-900/30 transition-colors cursor-pointer"
                                  >
                                    Adjust Balance
                                  </button>
                                  <button
                              ...
                              <tr className="bg-slate-50/70 dark:bg-slate-800/30">
                                <td colSpan={6} className="px-8 py-4">
                                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                    ...
                                    <div>
                                      <p className="text-gray-400 text-xs mb-1">POS Agents Referred</p>
                                      <p className="font-semibold text-gray-900 dark:text-gray-100">
          ...
          {tab === 'activations' && (
            <div className="card">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                POS agents whose KYC is approved but haven&apos;t reached 10 transactions yet. Override to manually activate and credit the BRM&apos;s ₦5,000 bonus.
              </p>
              {merchants.length === 0 ? (
                <div className="text-center py-16">
                  <CheckCircle className="w-10 h-10 text-green-400 mx-auto mb-3" />
                  <p className="text-gray-500 text-sm">No POS agents awaiting activation</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700">
                        <th className="pb-3 font-medium">POS Agent</th>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md border border-gray-200 dark:border-gray-700 p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">
                    Override POS Agent Activation
                  </h2>
                  <p className="text-xs text-gray-400">{overrideTarget.business_name}</p>
                </div>
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                This will mark the POS agent as <strong>activated</strong> and credit{' '}
                ...
                <div className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
                  ...
                  <div>
                    <p className="text-xs font-medium text-gray-400 mb-2 uppercase tracking-wide">
                      Referred POS Agents ({brmMerchants[profileTarget.id]?.length ?? '…'})
                    </p>
                    {!brmMerchants[profileTarget.id] ? (
                      <p className="text-sm text-gray-400">Loading…</p>
                    ) : brmMerchants[profileTarget.id].length === 0 ? (
                      <p className="text-sm text-gray-400">No POS agents referred yet.</p>
                    ) : (
      ...
      {editTarget && (
        <>
          <div className="fixed inset-0 bg-black/50 z-40" onClick={closeEditModal} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
            <form
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <button type="button" onClick={closeEditModal} className="text-gray-400 hover:text-gray-600 cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <label className="relative flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-blue-400 transition-colors overflow-hidden">
                      {editProfilePhotoPreview ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={editProfilePhotoPreview} alt="Profile preview" className="absolute inset-0 w-full h-full object-cover" />
                      ) : (
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => pickFile(e.target.files?.[0] ?? null, setEditProfilePhotoFile, setEditProfilePhotoPreview)}
                      />
                    </label>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ID Document Photo</label>
                    <label className="relative flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-blue-400 transition-colors overflow-hidden">
                      {editIdPhotoPreview ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={editIdPhotoPreview} alt="ID preview" className="absolute inset-0 w-full h-full object-cover" />
                      ) : (
                        <div className="flex flex-col items-center text-gray-400">
                          <Upload className="w-5 h-5 mb-1" />
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => pickFile(e.target.files?.[0] ?? null, setEditIdPhotoFile, setEditIdPhotoPreview)}
                      />
                    </label>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="email" value={editForm.email} onChange={(e) => setEditForm((f) => ({ ...f, email: e.target.value }))} className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                ...
                <p className="text-xs text-gray-400">Referral code cannot be changed.</p>
              </div>
